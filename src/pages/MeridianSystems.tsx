...
export const meridianSystems: SystemDefinition[] = [
  {
    ...
    summary: 'One owned destination for menu, reservations, events, location, WhatsApp and the identity of the room.',
    audience: 'A restaurant relying on social, Linktree and separate booking links should be able to land here and immediately recognize the next version of its own guest experience.',
    liveState: 'Tonight · 7:30 PM',
  ...
  },
  {
    ...
    audience: 'A local transportation operator should be able to see how riders, drivers, scheduled transfers and direct communication can live in one owned system.',
    liveState: '3 drivers available',
  ...
  },
  {
    ...
    audience: 'A cleaning company should be able to recognize its own day-to-day operation here: customers book, the team sees the work, and both sides know what happens next.',
    liveState: 'Next opening · 10:30 AM',
  ...
  },
  {
    ...
    audience: 'A tour operator should see how to keep WhatsApp where it helps while owning discovery, trust, logistics and the actual booking journey.',
    liveState: 'Tomorrow · 4 departures',
  ...
  return (
    <div style={shell}>
      ...
      <main style={{ position: 'relative', zIndex: 2, maxWidth: 'var(--container-max)', margin: '0 auto', padding: '48px var(--space-6) 96px' }}>
        <Link to="/" style={backLink}>← Meridian</Link>
        <div style={{ maxWidth: 760, padding: '88px 0 56px' }}>
          <Eyebrow tick>Systems in the Lab</Eyebrow>
          <h1 style={{ fontFamily: 'var(--font-serif)', fontSize: 'clamp(42px, 6vw, 72px)', fontWeight: 400, margin: '20px 0' }}>See the business inside the system.</h1>
          <p style={{ color: 'var(--text-secondary)', fontSize: 'var(--text-lead)', lineHeight: 'var(--leading-relaxed)', maxWidth: 680 }}>
            Working concepts for businesses whose real-world operation is stronger than the digital layer around it.
          </p>
        </div>
          ...
          {meridianSystems.map((system) => (
            <Link key={system.kind} to={`/systems/${system.kind}`} style={{ textDecoration: 'none', color: 'inherit' }}>
              <Card>
                ...
                <div style={{ marginTop: 24, fontFamily: 'var(--font-mono)', fontSize: 'var(--text-xs)', color: 'var(--text-gold)', textTransform: 'uppercase', letterSpacing: 'var(--tracking-wide)' }}>Open prototype →</div>
              </Card>
            </Link>
          ))}
        </div>
...
export function MeridianSystemPrototype({ kind }: { kind: MeridianSystemKind }) {
  ...
  return (
    <div style={shell}>
      <FieldBackground particleDensity="moderate" showPolyhedronMark={false} />
      <main style={{ position: 'relative', zIndex: 2, maxWidth: 'var(--container-max)', margin: '0 auto', padding: '48px var(--space-6) 96px' }}>
        <Link to="/systems" style={backLink}>← All systems</Link>
        <section style={{ padding: '72px 0 40px' }}>
          ...
          <p style={{ color: 'var(--text-secondary)', fontSize: 'var(--text-lead)', lineHeight: 'var(--leading-relaxed)', maxWidth: 760 }}>{system.summary}</p>
        </section>

        <section style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: 'var(--space-4)', marginBottom: 'var(--space-6)' }}>
          <Card>
            ...
            <div style={{ fontFamily: 'var(--font-serif)', fontSize: 'var(--text-title-2)', marginTop: 12 }}>{system.liveState}</div>
          </Card>
        ...
        <section style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: 'var(--space-6)', marginBottom: 'var(--space-7)' }}>
          <Card>
            <Eyebrow muted>Customer side</Eyebrow>
            <div style={{ display: 'grid', gap: 12, marginTop: 20 }}>
              {system.actions.map((action, index) => (
                <div key={action} style={{ textAlign: 'left', padding: '16px 18px', borderRadius: 16, border: '1px solid var(--border-subtle)', background: index === 0 ? 'var(--gold-500)' : 'transparent', color: index === 0 ? 'var(--text-on-gold)' : 'var(--text-primary)', fontFamily: 'var(--font-body)' }}>{action}</div>
              ))}
            </div>
          </Card>
          <Card>
        ...
        <section style={{ maxWidth: 820, padding: '48px 0 16px' }}>
          ...
          <h2 style={{ fontFamily: 'var(--font-serif)', fontWeight: 400, fontSize: 'var(--text-display-3)', margin: '16px 0' }}>The point is recognition, not a template.</h2>
          <p style={{ color: 'var(--text-secondary)', fontSize: 'var(--text-lead)', lineHeight: 'var(--leading-relaxed)' }}>{system.audience}</p>
          <p style={{ color: 'var(--text-muted)', lineHeight: 'var(--leading-relaxed)' }}>Meridian scopes each build around the actual operation. These prototypes show the shape of a system, not a package every business is forced into.</p>
        </section>
      </main>
    </div>
  );
